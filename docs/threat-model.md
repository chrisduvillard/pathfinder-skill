...
| Split or fabricated merge evidence | A pure uncalled composer requires the verified observer receipt and identity audits, authenticated publication receipt, fixed GraphQL projection, reconciled REST reviews, policy-derived required checks, exact check/status pages, globally unique request ids, and a canonical branch-ownership proof. The ownership proof requires active restricted create/update/delete rules, exactly the publication App as sole always-bypass actor, complete exact-branch effective rules, and a final ref/SHA reread after evidence completion. | No installed host collects, authenticates, or durably persists the publication journal, ownership proof, or evidence/provenance pair. |
