...
| Duplicate PR after crash | Exact head/base/mission lookup and persistent lost-response fixtures reuse one PR record; dispatch is durable but the lock is released before the remote callback, so process death cannot strand read-only recovery. The zero-merge source orchestrator has a separate recovery operation that can only call this read-only reconciliation before collection. | Publication is not composed into the enabled bridge. |
| Fabricated or rediscovered PR identity | The uncomposed publication prerequisite accepts only a fresh authenticated request with its full explicit one-PR authorization canonically bound, takes freshness only from its injected host clock, atomically spends that authorization/candidate, requires an exact read-only target preflight before mutation, and persists a closed receipt with repository plus PR database/node/number, exact head/base SHAs, diff hashes, and every required check's context/App/head SHA. Pending recovery uses read-only exact lookup and never repeats push/create. Before evidence reads, the zero-merge orchestrator reloads that exact terminal journal and the collector requires the authenticated input request/dispatch/receipt to equal its validated canonical documents. | No live backend, credential loader, CLI, mission caller, or installed route constructs the source orchestrator; a future trusted host must authenticate the clock, preflight, lookup, receipt, and collection-input source. |
...
| Split or fabricated merge evidence | A pure uncalled composer requires distinct verified observer and merge identities, authenticated publication receipt, fixed GraphQL projection, reconciled REST reviews, policy-derived required checks, exact check/status pages, globally unique request ids, and a canonical branch-ownership proof. Only the verified merge actor drives membership reads and evidence evaluation. Before any GitHub read, the collector requires one externally authenticated, trusted-time-bound closed envelope for the journal, authority, credential receipts, policy read, and full Git-object evidence; loose, stale, wrong-store, malformed, re-hashed, expired, or correctly signed but internally split inputs block after nested hash/binding checks. The source-only immutable host store then externally authenticates one exact v3 output envelope containing the journal, all three non-secret credential receipts, ownership, evidence, and provenance; it rechecks hashes/identities and rejects observer/merge substitution, tamper, replay, replacement, or interruption. | No installed host injects the authenticator/key, receipt source, App JWTs, or observer token, calls the store, or collects live inputs; neither envelope contains a merge token, and Windows host ACL verification remains unimplemented and fails closed. |
...
| Fixture-to-production escalation | Default-off tests scan the CLI and packaged routes for publication/merge constructors, exact live backends, merge credentials, and merge commands; source review and green fixtures are explicitly non-authorizing. | A future K5 change must deliberately update the guard only after its runnable-host, live-evidence, rehearsal, specialized-agent quorum, and explicit user-authorization gates are satisfied. The runtime independent-human PR approval floor remains separate. |
| Compromised dependency | Two pinned direct validation dependencies, required CI, package smoke from the exact archive, version-pinned stable tags, and a no-rewrite release policy. | Transitive/platform supply-chain and privileged tag-rewrite risk remain; dependency updates require review. |
