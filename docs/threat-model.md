...
| Split or fabricated merge evidence | A pure uncalled composer requires the verified observer receipt and identity audits, authenticated publication receipt, fixed GraphQL projection, reconciled REST reviews, policy-derived required checks, exact check/status pages, globally unique request ids, and a canonical branch-ownership proof. A source-only immutable host store then externally authenticates one exact envelope containing the journal, both credential receipts, ownership, evidence, and provenance; it rechecks all hashes/identities and rejects tamper, replay, replacement, or interruption. | No installed host injects the authenticator/key, calls the store, collects live inputs, or owns the remaining policy/credential boundaries; Windows host ACL verification remains unimplemented and fails closed. |
