...
| Split or fabricated merge evidence | A pure uncalled composer requires distinct verified observer and merge identities, authenticated publication receipt, fixed GraphQL projection, reconciled REST reviews, policy-derived required checks, exact check/status pages, globally unique request ids, and a canonical branch-ownership proof. Only the verified merge actor drives membership reads and evidence evaluation. A source-only immutable host store then externally authenticates one exact v3 envelope containing the journal, all three non-secret credential receipts, ownership, evidence, and provenance; it rechecks hashes/identities and rejects observer/merge substitution, tamper, replay, replacement, or interruption. | No installed host injects the authenticator/key, receipt source, App JWTs, or observer token, calls the store, or collects live inputs; the envelope contains no merge token, and Windows host ACL verification remains unimplemented and fails closed. |
