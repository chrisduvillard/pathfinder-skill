...
| Fabricated or rediscovered PR identity | The uncomposed publication prerequisite accepts only a fresh authenticated request, atomically spends its mission authorization/candidate, and persists a closed receipt with repository plus PR database/node/number, exact head/base SHAs, diff hashes, and successful check observation. Pending recovery uses read-only exact lookup and never repeats push/create. | No live backend, credential loader, CLI, mission caller, or installed route exists; a future trusted host must authenticate the lookup and receipt source. |
