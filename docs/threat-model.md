...
| Split or fabricated merge evidence | A pure uncalled composer requires the verified observer receipt and identity audits, authenticated publication receipt, fixed GraphQL projection, reconciled REST reviews, policy-derived required checks, exact check/status pages, and globally unique request ids; its provenance receipt is canonically bound to the resulting evidence hash. | No installed host authenticates or persists the pair, and exclusive controller-branch ownership through the evidence instant remains unproven. |
