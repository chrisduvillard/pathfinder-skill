...
| Source-only authenticated host artifacts | immutable-envelope tests and the `host-artifact-store-contract` eval cover exact journal/policy/authorization/protected-policy/credential/ownership/evidence/provenance bindings, effective additive-policy hashing, external-authentication tamper rejection, pair authority/authenticator drift, wrong-store/renamed replay, owner/symlink/overlap checks, write interruption, concurrent write-once behavior, and the two unconstructed source boundaries that write one snapshot or read a pair; no authenticator/key, route constructor, complete normalized REST backend, or installed collector is claimed |
