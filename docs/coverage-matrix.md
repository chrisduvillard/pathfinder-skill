...
| Source-only authenticated host artifacts | immutable-envelope tests and the `host-artifact-store-contract` eval cover exact journal/policy/authorization/protected-policy/credential/ownership/evidence/provenance bindings, effective additive-policy hashing, external-authentication tamper rejection, pair authority/authenticator drift, wrong-store/renamed replay, owner/symlink/overlap checks, write interruption, concurrent write-once behavior, and the sole unconstructed read-only consumer; no authenticator/key, route constructor, or installed collector is claimed |
