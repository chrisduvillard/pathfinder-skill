...
| Source-only authenticated host artifacts | immutable-envelope tests and the `host-artifact-store-contract` eval cover pre-read authentication of the closed journal/authority/credential/policy-read/object-evidence input, zero-network rejection of re-hashed tamper and stale/wrong-store input, exact output journal/policy/authorization/protected-policy/credential/ownership/evidence/provenance bindings, effective additive-policy hashing, pair authority/authenticator drift, wrong-store/renamed replay, owner/symlink/overlap checks, write interruption, concurrent write-once behavior, and the two unconstructed source boundaries that write one snapshot or read a pair; the complete normalized REST backend is source-only, and no authenticator/key, credential or receipt loader, route constructor, or installed collector is claimed |
