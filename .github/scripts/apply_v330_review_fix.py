...
def indent_method(value: str) -> str:
    return "".join("    " + line if line.strip() else line for line in value.splitlines(keepends=True))


storage_path = Path("pathfinder_core/storage.py")
storage = storage_path.read_text(encoding="utf-8")

old_import = "from .state import transition\n"
new_import = "from .state import ALLOWED_TRANSITIONS, transition\n"
...
peek_replacement = indent_method(
    dedent(
        '''\
        def peek(self) -> dict:
            """Read and validate canonical state without performing recovery or writes."""
...
if storage.count(peek_fragment) != 1:
    raise SystemExit("peek fragment did not match exactly once")
storage = storage.replace(peek_fragment, peek_replacement, 1)

load_fragment = indent_method(
    dedent(
        '''\
...
load_replacement = indent_method(
    dedent(
        '''\
        def _load_locked(self, *, recover: bool) -> dict:
            document = read_json(self.state_path)
...
if tests.count(footer) != 1:
    raise SystemExit("test footer did not match exactly once")
