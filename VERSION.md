...
Changes in v3.0.0:
- Hardened the still-inert conditional-merge evaluator after independent review: authorization now binds the exact controller PR/refs/diff, authenticated Git-object receipt, and low-risk Goal class; protected policy is anchored to the shipped baseline with additive-only overrides; protected/special surfaces, host policy reads, freshness, human/check identities, classic-protection semantics, and ruleset bypass modes are explicit; and only a replayed two-snapshot readiness proof over both persisted evidence documents—not a single advisory verdict—can satisfy the future intent/result schemas. Deterministic adversarial tests and an actual-code offline eval cover the reproduced bypasses. The fail-closed acceptance matrix links 16 observer/evaluator/inert-composition cases to direct executable evidence, keeps positive team/role/admin membership resolution as an explicit K3 follow-up, and leaves the two writer-only response-loss/reconciliation cases deferred to K4. No merge route, credential, writer, or production caller was added, and any future cross-process proof consumer must authenticate a host-owned envelope.
...
Changes in v2.21.10:
- Hardened five issues found by dogfooding Pathfinder on its own repository, each with a failing-before/passing-after fixture. (1) `check-skill-behavior.sh` gained a boundary-heading existence guard for the Family-A window (`## Autonomous mode`/`## Phase 7:`): a heading rename previously left the window unset so all four safety-direction guards passed vacuously (fail-open) — it now fails closed. (2) The same harness now direction-guards the `## Stop conditions` autonomous carve-out (self-merge stays conditional, the trust-boundary/carve-out stays "never waived"), which sat outside the Family-A window and was unchecked (SEC-1). (3) A forbidden-inversion guard closes the multi-qualifier evasion for the flagship self-merge control: an inverted clause on a saturated line (e.g. `unconditional self-merge` while `default-deny` survives) passed the whole-line check but is now caught (the polarity-inversion class that reached `main` in v2.21.1/.2). (4) `check-manifests.sh` scopes `MSYS_NO_PATHCONV=1` to its `/pathfinder charter` jq `--arg` call, so the Codex-prompt check no longer falsely fails on Windows/Git-Bash while Linux CI stays green (BE-1). (5) Every Explore drill-down level (L1-L4) now renders both universal escapes — the `ignore objectives` charter-bias off-switch (promised "at any level" but only on L0) and the canonical `describe your own` free-text escape — mirrored into `question-funnel-template.md` and enforced by a new per-level guard in `check-skill-consistency.sh` (FE-1/FE-2). Also: `CONTRIBUTING.md` and `.github/pull_request_template.md` now list all five checks `check-all.sh` runs (they omitted `check-skill-behavior.sh` and/or `test-validators.sh`). New meta-tests in `test-validators.sh` cover behaviors 4-7 and parser 7. Verified by Phase 4b adversarial panels (18 blind verifiers); a sixth candidate (clarity-gate triple-definition sync) was rejected because a marker-level guard already exists.
...
- Clarified README and the question-funnel reference so autonomous mode is described as hands-off after charter preflight, not as skipping every possible question.
- Added drift guards for the autonomous charter preflight and charter-reuse notice so future wording changes cannot silently break the behavior contract.
