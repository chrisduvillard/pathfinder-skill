...
- **Phase 6** — mirror the assembled goal back as the recognition-first, line-by-line contract. On a full-plugin single-Goal path, put that exact complete condition in `.prompt-goal-request.json`; the controller generates `06-goal-command.md` with both the `/goal` command and Implementation Goal fallback. A manual skill-only install writes the Markdown directly. A numbered goal pack remains on the manual route until the controller supports it.
