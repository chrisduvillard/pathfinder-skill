...
For an explicitly approved `run all` pack, keep each canonical schema-valid binding and its controller views under `goals/NNNN/` in the approved order. The host-owned pack state stores the exact mission/binding/Goal identities, canonical binding hashes, active index, pack deadline, and child mission paths. Only `pack-status` reads that queue as authority. Top-level pack Markdown is a human view and never supplies queue membership, order, completion, or authorization.
