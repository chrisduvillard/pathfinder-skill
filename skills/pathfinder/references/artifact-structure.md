...
In autonomous mode (see “Autonomous mode” in `SKILL.md`), the same numbered files are reused: `00-session.md` records the mission worktree path (`<repo-parent>/.pathfinder-worktrees/<repo-name>-<timestamp>-auto` or the ignored local fallback), `04-question-funnel.md` / `05-user-answers.md` record the selection or Autonomous Opportunity Scout derivation from the sanitized creator model/roadmap/doctrine, the ambiguity ledger and each loop pass, and any exclusions; `07-run-log.md` records the per-goal execution loop (mission worktree, branch, commands, exit results, verifier verdict, Runtime Boundary, Binding Status, push/PR/merge outcome) plus roadmap and doctrine updates; `07b-cross-model-review.md` records any optional Cross-Model Review packet, launch mode, verdicts, fixes, and disposition before publication; and `08-final-summary.md` adds the final shipped/blocked ledger and auto-resume ledger (one row per goal: autonomous disposition, the item's roadmap class (`autonomous-eligible` or `manual-approval`/`manual-only`), the needs-review reason when applicable, Binding Status, PR URL, CI status, verification verdict, cross-model review disposition when run, and the next input for anything not merged). A goal's **autonomous disposition** is one of `merged` (a goal that passed the branch-protection self-merge gate), `awaiting-review` (committed, pushed, and opened as a PR for human review, including when absent branch protection produces awaiting-review), `blocked` (a per-goal block before commit/push, including an item deferred on creator input such as an unanswered converted Open Question), or `excluded` (an item that was never worked, with its reason).
