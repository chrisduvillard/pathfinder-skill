...
The Deep Intent Gate and Doctrine Interview introduce no new numbered artifact: `04-question-funnel.md` / `05-user-answers.md` record the evidence draft, first-run interview, Project Doctrine screens, the ambiguity ledger and each loop pass, reconcile screens, refresh answers, explicit creator confirmation, and any `continue later` partial state. `00-session.md` records schema validation plus the `completion` and `intent_clarity` values and hashes for all three canonical intent JSON documents, the ignore decision, activation result, and backup location. `.pathfinder/charter.json`, `.pathfinder/roadmap.json`, and `.pathfinder/doctrine.json` are separate stable, local-only, never-committed canonical documents outside the run folder; matching `.md` files are replaceable controller-rendered human views. None are part of the 00-08 artifact set, and no numbered artifact or Markdown view may be parsed into canonical intent.
