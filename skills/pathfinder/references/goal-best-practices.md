...
- For autonomous goals, include a compact `Execution eligibility` contract line and supporting note showing the base-bound model-depth proof result: creator-intent status, doctrine alignment, repo evidence map, safety fit, protected-area proof, implementation boundary, verification plan, and blocking unknowns.
