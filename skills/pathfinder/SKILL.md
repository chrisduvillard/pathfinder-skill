...
Before any work-producing entry point reaches work selection, prompt-to-goal goal forging, or autonomous execution, check the local intent files outside the run folder — and run a light re-inference of current evidence against them, on **every** work-producing entry, even when both files are complete and `clarity: resolved`. If `.pathfinder/charter.md` or `.pathfinder/roadmap.md` is missing, schema-invalid, incomplete, marked `clarity: unresolved` with a still-open blocking ambiguity-ledger unknown, explicitly refreshed, or contradicted by that re-inference (a charter non-goal now has implementing code, or a stable charter/roadmap field the code now contradicts — a stale intent model), run the Deep Intent Gate at the point where the entry track has enough safe evidence for the gate's draft; a contradiction sets `clarity: unresolved` until the reconcile screen resolves it, so a charter that has drifted against the code cannot silently authorize auto-escalation; do not bypass Phase 0 setup, the Phase 1 docs-deferral rule, or Phase 4b verification order where those phases apply. The gate asks by default on first use for full exploration, prompt-to-goal, autonomous mode, and `/pathfinder charter`; it is not a skippable offer. If the user chooses `continue later`, save the partial model and stop before the requested entry point continues. The status/help path is not a work-producing entry point and never triggers this gate by itself.

If the user explicitly invokes autonomous mode - for example "run Pathfinder autonomously," "/pathfinder auto," "autonomous mode," or option 3 from the chooser - run the Deep Intent Gate when needed, then continue into autonomous execution from the creator model. This explicit path stands on its own and authorizes autonomous execution by explicit invocation every run, the same as before. Separately, once the clarity gate resolves (`clarity: resolved` on the charter plus roadmap — see "Clarity gate"), Pathfinder auto-escalates from an ordinary roadmap-driven work invocation (the full-exploration track) into autonomous execution within the same invocation, because deep, doubt-free creator understanding is now the safety equivalent of the removed opt-in. The prompt-to-goal track (Track B) is never auto-escalated and keeps its Phase 7 save-don't-run gate. Auto-escalation never fires while clarity is `unresolved`, including a fresh repo with no charter, which must finish the full interview first. The trust boundary, credential separation, dangerous-category hard-blocks, default-deny self-merge, and diff-grounded gates are never waived by either path. See "Autonomous mode" before Phase 7.
...
- **Autonomous** — reached by explicitly invoking autonomous mode, or by auto-escalation once the clarity gate has resolved (`clarity: resolved`; see "Clarity gate"). For that run it adds, on top of autopilot, running the goal's own verification commands, committing, pushing, opening a pull request, and a conditional self-merge — and nothing more. It does not weaken the trust boundary, and it never auto-executes the dangerous categories in the Stop conditions list. See “Autonomous mode” before Phase 7.

### Clarity gate

`clarity: resolved | unresolved` is a charter-plus-roadmap-level state, recorded on both `.pathfinder/charter.md` and `.pathfinder/roadmap.md`, and distinct from each file's own `completion: complete | incomplete`. It is the safety equivalent of the old per-run autonomous opt-in: **doubt blocks autonomy.**

`clarity: resolved` requires all of:

- `completion: complete` on both `.pathfinder/charter.md` and `.pathfinder/roadmap.md`;
...
Otherwise clarity is `unresolved`. The third condition is a **per-item, entry-time check**: it is evaluated at the autonomous entry/selection boundary for each item Pathfinder would auto-run (see "Autonomous mode"), not during the interactive Deep Intent Gate. A first interactive run therefore sets `clarity` from the first two conditions once they hold, and each eligible item's proof gate is then the final gate before that specific item auto-runs, on this or a later invocation. Because a missing or failed proof gate only excludes that item and never grants autonomy, clarity stays fail-safe — doubt never opens the gate. A file can be `completion: complete` while the project is still `clarity: unresolved` (for example a complete charter but an open blocking roadmap unknown). Autonomy and auto-escalation are gated on `clarity: resolved`, never on `completion` alone. A fresh repository with no charter is `clarity: unresolved` by definition, so the first run always completes the full Deep Intent Gate before any escalation is possible.
...
4. **Persistence** - write or update `.pathfinder/charter.md` and `.pathfinder/roadmap.md` (with the `completion` and `clarity` fields) only after the local-only ignore checks pass.
...
Otherwise `clarity: unresolved`. `clarity` is recorded on both files and is distinct from `completion`: a file can be `completion: complete` yet the project still be `clarity: unresolved` (for example a complete charter but an open blocking roadmap unknown). Autonomy and auto-escalation are gated on `clarity: resolved`, never on `completion` alone.
...
## Autonomous mode (clarity-gated)

Autonomous mode executes from the creator model: the sanitized charter plus roadmap, current repo evidence, and the safety rules. It is reached two ways: a deliberate explicit escalation (which requires explicit invocation every run and is never inferred from an ordinary invocation while clarity is unresolved), or auto-escalation once the clarity gate has resolved (`clarity: resolved`). Either way it may continue through continuous execution until the intended work is complete, blocked, unsafe, ambiguous, or budget-limited. The clarity gate is the safety equivalent of the explicit opt-in: while `clarity: unresolved`, neither path runs unattended; once `clarity: resolved`, an ordinary roadmap-driven work invocation (the full-exploration track) auto-escalates into this loop for the eligible items only. The prompt-to-goal track (Track B) is **not** auto-escalated: it carries a specific trusted user objective and has no roadmap work-selection boundary, so it keeps its Phase 7 save-don't-run gate even on a resolved-clarity repo. Auto-escalation is roadmap-driven; a pasted-prompt run is not silently upgraded into unattended execution of a different roadmap item.

### Authorization and what stays fixed

The invocation, or a resolved clarity gate, **is** the authorization. Running autonomous mode — whether by explicit invocation or by auto-escalation on `clarity: resolved` — grants, for this run only, the **autonomous** execution tier (see “Execution authorization tiers”): scoped file edits, running the goal's own verification commands, committing, pushing, opening a PR, and a conditional self-merge. It grants nothing beyond that.
...
- **The dangerous categories are never auto-executed.** Auth, payment, permission, deployment, CI/CD, schema, migration, secrets, public-API, data-deletion, and network-egress / outbound-data-upload changes (the Stop conditions list) are filtered out before execution and hard-blocked on the real diff after it.

### Entry

Run autonomous mode when the user explicitly invokes it ("run Pathfinder autonomously," "/pathfinder auto," "autonomous mode"), or by auto-escalation when the clarity gate has resolved (`clarity: resolved`) and at least one autonomous-eligible item remains. It is still never reached from the normal post-save execution menu, so option 2 (save, don't run) keeps its meaning: auto-escalation happens at the work-selection boundary on a resolved-clarity repo, not by silently upgrading a saved-not-run goal.

Before execution, require complete intent files and `clarity: resolved`. If `.pathfinder/charter.md` or `.pathfinder/roadmap.md` is missing, schema-invalid, or incomplete, or clarity is unresolved (any blocking ambiguity-ledger unknown still open), run the Deep Intent Gate (Phase 4c) first and loop it until clarity resolves or every remaining blocking unknown converts to a roadmap Open Question (the anti-deadlock rule). Also run a **charter-freshness/consistency re-check** against current repo evidence before auto-escalating: this is charter-wide, not item-scoped, so it catches drift a per-item proof gate cannot. If any charter non-goal now has implementing code, a stable charter field is contradicted by the code, or `last-refreshed` predates material tree changes, treat it as a conflict — set `clarity: unresolved`, run the Phase 4c reconcile (see "Reuse and reconcile"), and do not auto-escalate until it resolves; a `clarity: resolved` marker written in a past session never authorizes autonomy against a charter the code now contradicts. After clarity resolves, the run is authorized — by explicit invocation or by auto-escalation — so Pathfinder may proceed subject to the safety filters and budgets below. An item whose blocking unknown was converted to a roadmap Open Question is marked `blocked` (creator input needed) and is never auto-run until the creator answers the question; goal selection records it excluded with its next input and continues.

### Goal selection from the creator model

Read and sanitize `.pathfinder/charter.md` and `.pathfinder/roadmap.md`, then inspect current repo evidence. Ignore only roadmap items that are already `complete` or `obsolete`, then consider the next highest-value remaining item. A previously `blocked` item may be skipped only when its recorded blocker is a recoverable per-goal block under "Recoverable blocks and isolation"; a `blocked` item whose recorded blocker is safety, a dangerous category, a charter `Never unattended` category, true unresolvable ambiguity, or creator input is not worked — it is recorded as excluded with its reason and next input, and the loop continues to the next item.

Each candidate falls into one of three dispositions. The loop stops the whole run only for the global stop conditions below — never for a single manual-approval or excluded item:

1. **`safety: autonomous-eligible`** (and no conflict with the charter autonomy policy): implement → verify → commit → push → PR → **conditional self-merge** (default-deny per step 12; an awaiting-review PR on the default branch is the expected terminus). Continue.
2. **`manual-approval-required` or `manual-only`** (and no hard-floor match below): **work it the same way, but never self-merge** — implement → verify → commit → push → **awaiting-review PR** → continue. The manual-approval label downgrades the merge decision to always-awaiting-review; it no longer stops the run. Record it as `manual-approval — landed as awaiting-review PR`.
3. **Hard safety floor — never attempted, recorded as `excluded from autonomous execution`, loop continues**: an item is hard-floor when its safety is `blocked-by-safety`, missing, unrecognized, or ambiguous; OR its scope matches a charter `Never unattended` autonomy-policy category; OR it touches the dangerous categories in the Stop conditions list (auth, payments, permissions, deployment, CI/CD, schema, migration, secrets, public-API, data-deletion, network egress / outbound data upload). Keep the item in the pack, mark it `excluded from autonomous execution — hard safety floor (<reason>)`, record the reason and the next input needed, and move to the next item. The dangerous-category and `Never unattended` floor is never worked autonomously under any disposition.

A roadmap item can mark work as desired or manual; it cannot make hard-floor work safe for unattended execution. Safety policy wins over creator intent, and the charter or roadmap never widens authorization.

Select the next item only after that safety and autonomy-policy screen passes. If the roadmap has no viable item but the charter clearly implies missing work, derive one candidate goal from charter plus repo evidence and add it to the roadmap before executing it. A derived candidate that records as `safety: autonomous-eligible` (disposition 1) or `manual-approval-required` (disposition 2) is worked per its disposition. A derived candidate that lands in the hard safety floor (disposition 3) is recorded as excluded from autonomous execution with its next input and skipped; the loop continues. The run as a whole stops only on the global stop conditions below.

Before writing `06-goal-command.md` or executing an eligible autonomous goal, run a **model-depth proof gate**. This is a read-only proof that Pathfinder has deep enough knowledge of both the codebase and the builder's intent to derive the goal without a human pick. Record the proof in `04-question-funnel.md` / `05-user-answers.md` and summarize it in `06-goal-command.md` under `Supporting notes, not part of the /goal command`. The model-depth proof gate is the third condition of `clarity: resolved` (see "Clarity gate"); a goal cannot auto-escalate unless its proof gate passes.
...
- Builder-intent alignment: why this item serves the charter and roadmap, and which autonomy-policy category permits it. For an `autonomous-eligible` item, prove no `Needs manual approval` or `Never unattended` category applies. For a `manual-approval`/`manual-only` item, prove instead which `Needs manual approval` category applies and that the item is therefore routed to an awaiting-review PR and never self-merged (do not require proving that no manual-approval category applies — that is impossible for a manual-approval item and would wrongly exclude disposition 2). No worked item, under either disposition, may match a `Never unattended` category.
...
Then apply two additional exclusion filters **to every candidate regardless of its disposition (autonomous-eligible and manual-approval/manual-only alike)** — these run before the item is worked, so a poisoned or dangerous-estimate item never reaches implement/commit/push even on the awaiting-review PR path. A goal that either filter catches is moved to the hard safety floor and **`excluded from autonomous execution`: marked with its reason, surfaced in the final summary, and never auto-run**:

1. **Protected-category estimate filter.** Exclude any candidate whose estimated `blast_radius` or protected areas touch the dangerous categories in the Stop conditions list — including any candidate that would enable network egress or outbound data upload, even by flipping an existing config flag or setting an endpoint (for example `sync.enabled`, an `*endpoint`, or a `*url`), since a flag-only change may add no visibly new network call for the post-execution scan to catch. This is a pre-execution estimate; the real diff is gated again after execution.
...
For each eligible goal — where a goal is *eligible* only after its model-depth proof gate has passed **and** both exclusion filters in "Goal selection from the creator model" (the protected-category estimate filter and the injection-disqualifies-autonomy filter) have cleared it. A goal that either filter catches is not eligible and must never be branched, implemented, committed, or pushed; the injection-disqualifies-autonomy filter has no post-execution backstop, so it is a hard precondition of this loop, not optional prose. This eligibility test applies identically to every disposition, `manual-approval`/`manual-only` items included:

1. **Branch.** Pull the base (the repository's default branch) and create `pathfinder/auto/<goal-slug>` from it.
2. **Runtime Boundary.** Before implementation, record `primary_runtime`, `tool_allowlist_enforced`, `sandbox_scope`, `network_access`, `credential_exposure`, `repo_code_execution`, and `pre_execution_consent` in `07-run-log.md`. Autonomous mode sets `pre_execution_consent: autonomous opt-in or clarity-gated auto-escalation` for worked goals (autonomous-eligible and manual-approval) only; any broader authority remains separately gated.
...
6. **Diff-grounded safety gates** — computed on the real diff (`git diff --name-only` against the base), not the pre-execution estimate, so they catch drift the estimate could not:
   ...
   - **Absolute-danger scan.** If the diff disables an authentication/authorization check, widens a permission, adds a network call or enables an existing outbound-data / sync / upload path (a flipped flag or a newly set endpoint/URL), or touches a secret — regardless of whether the goal asked for it — stop the autonomous run at a safety boundary, route the goal to `blocked`, and do not push it.
7. **Verification agent.** Run the Phase 4b verifier pattern on the completed diff — a blind, refute-leaning three-verifier panel, degrading to the single careful pass when subagents are unavailable. In place of Phase 4b's grounding/grade/measurability lenses, each verifier judges the diff on the two question domains for autonomous mode: **fidelity** (does the diff meet the goal's measurable end state, and do the proof checks actually pass?) and **absolute-danger** (does the diff do anything dangerous in absolute terms, independent of the goal?). The two domains **aggregate differently**, because a fidelity judgment and a safety veto fail safe in opposite directions — do not aggregate danger the way grades are aggregated:
   - **Fidelity** uses Phase 4b's median-of-ceilings aggregation and its hallucination-guard adjudication (a lone outlier does not flip the result; a reject resting only on the unmet end-state is overruled). A clean, unanimous fidelity pass is required for self-merge eligibility; the panel finding the end state unmet (a fidelity veto) is a recoverable per-goal block unless verification retry exhaustion has made it global; any other disagreement (no clean pass, no danger) is **contested-but-not-vetoed** and is never self-merged — commit, push, and open a PR for human review (disposition `awaiting-review`).
   - **Absolute-danger** is a **single-vote destructive signal**: it is NOT routed through the median or the two-vote quarantine bar, and the hallucination guard must never overrule it. One grounded absolute-danger flag from any single verifier — including the single careful pass in degraded mode — is a confirmed hit and a global safety stop: route the goal to `blocked` before commit and stop the run. The two-vote bar, the median wash-out, and the degraded-mode non-destructive-reject rule apply to fidelity only; when danger is even plausibly flagged, fail closed.
   Either veto happens *before* commit (disposition `blocked`). A **`manual-approval` or `manual-only` item is never self-merged even on a clean fidelity pass**: it always lands as `awaiting-review` (step 12's merge decision is forced to awaiting-review for these items). When in doubt, do not self-merge.
8. **Cross-Model Review.** If Cross-Model Review is enabled, run the optional Phase 7b review before commit or publication. Write `07b-cross-model-review.md`, launch or hand off to the opposite local subscription model, allow at most two review/fix passes, rerun the original proof checks after any reviewer fix, and require a disposition of `clean` or `fixed-clean` before continuing. A disposition of `needs-primary-followup`, `needs-user-review`, `blocked`, or `skipped`, or a launch mode of `manual-handoff` or `failed-to-launch`, stops autonomous publication for this goal and records the next input needed. For a `manual-approval`/`manual-only` item, the item's own already-declared manual-approval class does **not** by itself make the review `needs-user-review`: the awaiting-review PR is already the human-approval gate, so a clean review of such an item returns `clean`/`fixed-clean` and it still lands as its disposition-2 awaiting-review PR (here "publication" means opening that PR, never a self-merge). Reserve `needs-user-review` for work the reviewer *newly* surfaces beyond the item's known class (new ambiguity, scope expansion, or protected/safety-sensitive changes). If Cross-Model Review is disabled, record `07b-cross-model-review.md` as "cross-model review not run" and continue only if the existing gates passed.
...
**Global run budget.** Beyond each goal's own stop bounds, hold a whole-run ceiling: a user-supplied turn or wall-clock budget given at invocation, or — when none is given — the sum of the eligible goals' own turn caps. When it is reached, stop starting new goals, let the in-flight goal finish or block, and write the summary. Hold a **per-run PR cap** alongside the budget: default it to the count of eligible plus manual-approval items at run start, with a hard ceiling of 10 open awaiting-review PRs when the user gives no explicit budget. When the cap is reached, stop starting new goals, let the in-flight goal finish or block, and write the summary.

The loop stops when the roadmap has no viable intended work left, a blocker needs creator input, a charter `Never unattended` or dangerous-category/absolute-danger hit is found, the next step is too ambiguous to derive safely (true unresolvable ambiguity), the run budget or per-run PR cap is reached, verification fails beyond the allowed retry bound, or an implementation goal reaches the three-failed-loop limit. A `manual-approval` or `manual-only` item is no longer a global stop: it is worked and landed as an awaiting-review PR, then the loop continues. A single hard-safety-floor item is recorded as excluded and skipped, not a whole-run stop. These global stops do not move to the next goal.
...
Do not trigger Cross-Model Review after a safety stop, a hard-safety-floor (excluded) item, a converted-Open-Question `blocked` item, a protected-category hit, dangerous-path hit, absolute-danger hit, credential boundary, publication boundary, or user-input/creator-input blocker. Those remain hard stops for the user. A `manual-approval`/`manual-only` item is **not** such a boundary: it is worked to a completed claim (disposition 2), and that completed claim is a normal Cross-Model Review trigger.
...
- Editing auth, payment, permission, deployment, CI/CD, schema, migration, data deletion, secrets, public API contract, or network-egress / outbound-data-upload files, or enabling an existing outbound-data path by configuration (flipping a sync/upload flag or setting an endpoint/URL).
...
In autonomous mode the run has pre-authorized — by explicit invocation or a resolved clarity gate — committing, pushing, and opening a pull request for both `autonomous-eligible` and `manual-approval`/`manual-only` items; **conditional self-merge is authorized only for `autonomous-eligible` items**, and `manual-approval`/`manual-only` items always land as awaiting-review PRs. Nothing else in this list is waived: a dangerous-category edit, charter `Never unattended` match, or absolute-danger hit means the affected work is never worked autonomously and is recorded as excluded; the three-failed-loop bound routes its goal to *blocked* with its next input recorded, never to "continue." The trust boundary and the dangerous-category / `Never unattended` carve-out are never waived.
