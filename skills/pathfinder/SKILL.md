...
If the user explicitly invokes autonomous mode — for example “run Pathfinder autonomously,” “/pathfinder auto,” or “autonomous mode” — run the full exploration normally, then auto-select and execute the verified Top 5 end to end (implement, verify, commit, push, open a pull request, and self-merge where the repository’s own rules allow it) without further approval. Autonomous mode is an explicit opt-in escalation of the default; never infer it from an ordinary invocation. See “Autonomous mode (opt-in)” before Phase 7.
...
5. **Verification agent.** Run the Phase 4b verifier pattern on the completed diff: a blind, refute-leaning, three-lens panel with the same median-of-ceilings aggregation and hallucination-guard adjudication, degrading to the single careful pass when subagents are unavailable. Two lenses for autonomous mode: **fidelity** (does the diff meet the goal's measurable end state, and do the proof checks actually pass?) and **absolute-danger** (does the diff do anything dangerous in absolute terms, independent of the goal?). A contested verdict blocks the merge — mirror the single-pass rule where a reject caps the outcome; when in doubt, do not merge. A veto blocks the goal.
6. **Commit** the diff on the branch.
7. **Publish.** Introduce the push credential now, as a separate step after verification — push the branch and open a pull request.
8. **Wait for CI.** If required checks go red, block the goal.
9. **Merge — default-deny.** Self-merge requires a **positive branch-protection signal**, never the mere absence of a blocker. Query the base branch's protection (for GitHub, `gh api repos/{owner}/{repo}/branches/{base}/protection` against the actual PR base, not an assumed `main`) and merge only when protection exists, its required status checks are green, and it does not require human review. Absence of protection, an auth/permission error, a non-GitHub remote, or no `gh` is **not** permission — leave the PR open and CI-green and report it as awaiting review (a shipped-to-PR outcome, not a block). A merge GitHub rejects at merge time (a race or a conflict) leaves the PR open, or blocks with “rebase” as the next input.
10. **Advance.** On a clean merge, the next goal branches from the now-updated base.

**Isolate and continue.** Any block — a stop bound hit, a CI failure, a verifier veto, a protected-path or absolute-danger hit, or a merge conflict — records the blocker and the next input needed, leaves the branch in place for inspection, and moves to the next goal. One goal never strands the rest.

**Global run budget.** Beyond each goal's own stop bounds, hold a whole-run ceiling (a total turn or wall-clock budget). When it is reached, stop starting new goals, let the in-flight goal finish or block, and write the summary.
